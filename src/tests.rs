use crate::{cpu, Byte, Memory, Word};

#[test]
fn test_inc_a_from_zero() {
    let mut mem = Memory::create();
    mem.write(0x0000, 0xA0);
    ...
    cpu::test_code(mem.as_mut(), 3, initial, expected);
}
...
fn test_mov_a_const_zero() {
    ...
    cpu::test_code(mem.as_mut(), 4, initial, expected);
}
...
fn test_transfer_gpr_to_spr() {
    let mut mem = Memory::create();
    mem.write(0x0000, 0x13);
    mem.write(0x0001, 0x18);
    mem.write(0x0002, 0x27);
    ...
    cpu::test_code(mem.as_mut(), 5, initial, expected);
}

#[test]
fn test_transfer_spr_to_gpr() {
    let mut mem = Memory::create();
    mem.write(0x0000, 0x28);
    mem.write(0x0001, 0x1B);
...
fn test_stack() {
    let mut mem = Memory::create();
    mem.write(0x0000, 0x72);
    mem.write(0x0001, 0x73);
    mem.write(0x0002, 0x7A);
    ...
    cpu::test_code(mem.as_mut(), 8, initial, expected);
}
